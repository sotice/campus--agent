...
| 可选展示 | 本地 Mock API Server，用于展示接口分层，不作为 P0 依赖。 |
...
| 输出 | PendingAction 状态、confirmationId、执行结果或取消/过期提示。 |
...
3. 用户确认时提交 `runId` 与 `pendingActionId`，Agent 通过 `resume(runId, pendingActionId)` 校验通过后签发一次性 `confirmationId`。
...
3. 本地课表可离线查看。
4. 课程新增、编辑、删除、冲突检测为 P1，不作为 P0 验收阻塞项。
