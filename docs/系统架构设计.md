...
sequenceDiagram
    ...
    Agent->>Gate: 校验 frozenParamsHash 和 confirmationId
    Gate-->>Agent: allow
...
| PendingAction 到 Tool | 敏感工具必须携带有效 `pendingActionId`、`confirmationId` 和匹配的 `frozenParamsHash`。 |
...
8. Android APK 至少跑通一次饭卡挂失闭环，作为移动端交付证据。
