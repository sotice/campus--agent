...
| 能力 | 示例 | MVP 要求 |
| --- | --- | --- |
| Observe | 读取“校园卡丢了，帮我挂失”的用户目标、演示账号、上下文 | P0A |
| Plan | 生成“查状态 → 确认 → 挂失 → 验证 → 查补办流程”计划 | P0A |
| 工具选择 | 选择 `schedule.query`、`campus_card.get_status`、`knowledge.search` | P0A |
| 参数抽取 | 从“明天上午有课吗”抽取日期和时间段 | P0A |
| Tool Safety Gate | 阻止未注册工具、非法参数、未确认敏感操作 | P0A |
| Agent Run 状态机 | 跟踪 runId、暂停确认、resume、终态 | P0A |
| PendingAction | 挂失前创建待确认动作，冻结参数哈希并显示风险 | P0A |
| Verify | 挂失后再次确认状态为 `lost`，知识回答校验 `score` | P0A |
| 多轮上下文 | “那怎么补办？”继承上一轮校园卡上下文 | P0A |
| RAG 可信问答 | 图书馆开放时间、校园卡补办流程带来源 | P0A |
| Debug Trace | 展示 phase、intent、tool、safety decision、duration | P0A |
...
4. `resume` 内部校验 PendingAction、`frozenParamsHash` 和 Run 状态，通过后签发一次性 `confirmationId`；该凭证只在 Orchestrator / Safety Gate / Tool Adapter 内部流转，前端和模型不可见。
5. `resume` 后只能继续原计划中被冻结的后半段，不得重新让模型改写执行参数。

## 4. 意图分类设计

| 意图编码 | 意图名称 | 示例语句 | 目标工具 | MVP |
| --- | --- | --- | --- | --- |
| `schedule_query` | 查询课表 | “我明天上午有课吗” | `schedule.query` | P0A |
| `campus_card_status` | 查询校园卡状态 | “我的校园卡还能用吗” | `campus_card.get_status` | P0A |
| `campus_card_report_loss` | 挂失校园卡 | “校园卡丢了，帮我挂失” | `campus_card.get_status` + `campus_card.report_loss` | P0A |
| `knowledge_qa` | 校园知识问答 | “图书馆今天几点关门” | `knowledge.search` | P0A |
| `procedure_query` | 办事流程查询 | “校园卡怎么补办” | `knowledge.search` | P0A |
| `smalltalk` | 普通聊天 | “你好” | 无工具 | P0A |
| `schedule_mutation` | 修改课表 | “帮我删掉这门课” | `schedule.create/update/delete` | P1 |
| `unknown` | 无法识别 | “帮我处理一下那个” | 澄清问题 | P0A |

## 5. 工具注册协议

每个工具必须在 Tool Registry 中声明名称、描述、输入参数、输出结构、敏感等级、权限要求、确认要求、环境限制和错误码。模型只能引用注册表中的工具名，并且只能生成 `modelInputSchema` 内的业务参数；`systemInjectedContext` 由 Orchestrator 在安全校验后注入，不进入 Prompt、前端状态或 Debug Trace。
...
```json
{
  ...
  "systemInjectedContext": {
    ...
    "properties": {
      ...
      "frozenParamsHash": {
        "type": "string",
        "description": "规范化执行参数的 SHA-256 哈希，必须与 PendingAction 冻结参数哈希一致"
      }
    },
    "required": ["pendingActionId", "confirmationId", "frozenParamsHash"],
...
| Schema 校验 | 模型参数必须符合 `modelInputSchema`；系统注入上下文必须符合内部执行约束 | `BAD_REQUEST` 或追问 |
| 权限检查 | 需要登录/演示账号时必须满足 | `UNAUTHORIZED` |
| 敏感等级 | `sensitive=true` 必须进入 PendingAction | `CONFIRMATION_REQUIRED` |
| 确认凭证 | `pendingActionId` 与内部 `confirmationId` 必须有效；客户端和模型不得提交 `confirmationId` | `PENDING_ACTION_EXPIRED` 或 `CONFIRMATION_REQUIRED` |
| 参数冻结 | 规范化执行参数哈希必须与 PendingAction 的 `frozenParamsHash` 一致；`frozenParamsSummary` 仅用于 UI 展示 | `SAFETY_GATE_BLOCKED` |
...
| 工具 | 敏感等级 | 必要保护 | MVP |
| --- | ---: | --- | --- |
| `campus_card.report_loss` | 高 | 登录/演示账号、PendingAction、二次确认、执行后验证、操作日志脱敏 | P0A |
| `schedule.delete` | 中 | 删除确认、撤销提示或备份 | P1 |
| `user.clear_data` | 高 | 二次确认、不可恢复提示 | P2 |
| `campus_card.get_status` | 中 | 登录/演示账号、脱敏展示 | P0A |
...
2. 用户确认时只提交 `runId` 与 `pendingActionId`，系统通过 `resume(runId, pendingActionId)` 校验后签发一次性 `confirmationId`。
3. `confirmationId` 只对该 PendingAction、该工具、该 `frozenParamsHash` 有效，并且只存在于内部执行上下文中。
...
1. 能稳定执行三条核心 Demo：校园卡挂失、查询课表、查询图书馆时间。
2. 校园卡挂失必须展示“查状态 → PendingAction → 确认 → 执行 → Verify → 补办来源”。
3. 未确认、取消、过期时不会调用 `campus_card.report_loss`。
4. 所有 P0A 工具调用均通过 Tool Safety Gate。
