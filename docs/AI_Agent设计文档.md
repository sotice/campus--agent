...
5. 敏感操作必须使用 PendingAction 协议，确认前不得执行。
6. 校园知识类问题必须优先使用知识库证据，并按置信度规则回答或拒答。
7. Demo 模式下提供脱敏 Debug Trace，证明系统具备 Agent 闭环而非聊天壳。
...
| PendingAction | 挂失前创建待确认动作，冻结参数并显示风险 | P0 |
...
```mermaid
flowchart TD
    A[用户消息] --> B[Observe: 读取上下文/用户状态/当前日期]
    ...
    I -- 确认 --> J[校验 PendingAction 并签发 confirmationId]
    J --> G
    G --> K[Verify: 校验结果/业务状态/证据置信度]
    K --> L{验证通过?}
...
3. `confirmationId` 只对该 PendingAction、该工具、该冻结参数有效。
...
4. 最终回复应附带来源卡：标题、来源、更新时间、可信等级、置信度。
...
| 知识库低置信 | 不编造，给官方查询建议 | 查看人工入口 |
| 本地课表为空 | 说明无数据，引导使用演示课表或添加课程 | 使用演示课表 |

## 13. Judge / Debug Trace 设计

Debug Trace 用于比赛演示，不是普通用户默认功能。它只展示阶段级事实，不展示隐藏推理链。
