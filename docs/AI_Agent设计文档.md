...
| 能力 | 示例 | MVP 要求 |
| --- | --- | --- |
| Observe | 读取“校园卡丢了，帮我挂失”的用户目标、演示账号、上下文 | P0 |
...
4. `resume` 内部校验 PendingAction、`frozenParamsHash` 和 Run 状态，通过后签发一次性 `confirmationId`。
...
| `campus_card_status` | 查询校园卡状态 | “我的校园卡还能用吗” | `campus_card.get_status` | P0 |
| `campus_card_report_loss` | 挂失校园卡 | “校园卡丢了，帮我挂失” | `campus_card.get_status` + `campus_card.report_loss` | P0 |
| `knowledge_qa` | 校园知识问答 | “图书馆今天几点关门” | `knowledge.search` | P0 |
| `procedure_query` | 办事流程查询 | “校园卡怎么补办” | `knowledge.search` | P0 |
...
```json
{
  ...
  "inputSchema": {
    ...
    "required": ["reason", "pendingActionId", "confirmationId", "frozenParamsHash"]
  },
  "outputSchema": {
    "type": "object",
    "properties": {
...
2. 用户确认时提交 `runId` 与 `pendingActionId`，系统通过 `resume(runId, pendingActionId)` 校验后签发一次性 `confirmationId`。
